...
import {
  ...
  getDispatcherNotificationMessage,
...
export function useNotifications(userId) {
  ...
  useEffect(() => {
    ...
    console.log('✅ Setting up notifications for dispatcher:', userId);
  ...
  const setupNotificationMonitoring = async () => {
    try {
      console.log(`🔍 Setting up notification monitoring for dispatcher: ${userId}`);
      console.log(`🔍 Channel name will be: dispatcher-notifications-${userId}`);
      console.log(`🔍 Listening for INSERTs on notifications where user_id=eq.${userId} and app_type=dispatcher`);

      // Subscribe to new notifications being inserted into notifications table
      const channel = supabase
        .channel(`dispatcher-notifications-${userId}`)
        .on(
          'postgres_changes',
          {
            event: 'INSERT',
          ...
          async (payload) => {
            ...
            // Only process dispatcher app notifications
            if (notification.app_type !== 'dispatcher') {
              console.log('⏭️ Skipping non-dispatcher notification');
              return;
            }
